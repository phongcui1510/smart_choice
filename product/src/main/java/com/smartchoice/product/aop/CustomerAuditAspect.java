...
public class CustomerAuditAspect {
    ...
    @Before("execution(* com.smartchoice.product.controller.ProductController.getProductByKeyword(..))")
    @Async
    public void logUserViewProduct(JoinPoint joinPoint) {
        Object[] methodArguments = joinPoint.getArgs();
        String keyword = (String) methodArguments[0];
        String username = (String) methodArguments[1];
        CustomerActivity customerActivity = new CustomerActivity(username, LocalDateTime.now(), keyword);
        System.out.println("customerActivity::"+customerActivity.getSearchKeyword());
        Message<CustomerActivity> message = MessageBuilder.withPayload(customerActivity).build();
        System.out.println("Sending to queue::"+message.getPayload().getSearchKeyword());
